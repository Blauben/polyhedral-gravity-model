...
struct IFunction {
    ...
    explicit IFunction(const std::string_view name) : name(name) {}
};
...
struct Function final : IFunction {
    ...
    Function(const std::string_view name, std::function<ReturnType(Args...)> func, Args... values) : IFunction(name), func {func}, values{values...} {}
